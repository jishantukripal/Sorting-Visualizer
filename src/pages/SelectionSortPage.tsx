...
import { generateSelectionSortSteps, SELECTION_SORT_CODE } from '../lib/selectionSort';
...
const LEGEND = [
  ...
  { label: 'Found Min', color: 'bg-success' },
  { label: 'Swap', color: 'bg-danger' },
  { label: 'Sorted', color: 'bg-secondary/20' },
];

export const SelectionSortPage: React.FC = () => {
  const viz = useVisualizer(generateSelectionSortSteps);
      ...
      <div className="w-full lg:w-96 bg-surface lg:border-l border-border-color flex flex-col h-auto lg:h-full">
        <div className="flex-1 p-6 flex flex-col">
            ...
            <div className="flex-1">
                <CodeViewer currentLine={viz.currentStep.lineNo} codeLines={SELECTION_SORT_CODE} />
            </div>
        </div>
      </div>
    </div>
