...
import { generateBinarySearchSteps, BINARY_SEARCH_CODE } from '../lib/binarySearch';
import { Visualizer } from '../components/Visualizer';
import { Controls } from '../components/Controls';
import { CodeViewer } from '../components/CodeViewer';

const LEGEND = [
    { label: 'Mid Point', color: 'bg-accent' },
    { label: 'Found', color: 'bg-success' },
    { label: 'Eliminated', color: 'bg-secondary/20' },
];

export const BinarySearchPage: React.FC = () => {
  const viz = useVisualizer((arr, target = 10) => generateBinarySearchSteps(arr, target), 20, true);
      ...
      <div className="w-full lg:w-96 bg-surface lg:border-l border-border-color flex flex-col h-auto lg:h-full">
        <div className="flex-1 p-6 flex flex-col">
            <h3 className="text-lg font-bold text-main mb-4 flex items-center gap-2">
                Binary Search
                <span className="text-xs font-normal text-secondary border border-border-color px-2 py-0.5 rounded-full">O(log n)</span>
            </h3>
            <div className="flex-1">
                <CodeViewer currentLine={viz.currentStep.lineNo} codeLines={BINARY_SEARCH_CODE} />
            </div>
        </div>
      </div>
    </div>
